...
상권의 유동인구, 연령대, 성별, 시간대별 분포를 분석합니다. 업종 적합도 점수 제공. *(통계 기반 추정)*
...
최근 창업 트렌드와 업종별 성장/쇠퇴 현황을 분석합니다. *(SEMAS + 통계 기반)*
...
#### 10. `get_nearby_facilities` - 주변 편의시설 분석
상권 주변 편의시설을 분석합니다. 지하철역, 버스정류장, 주차장, 은행, 편의점 등 접근성 평가.

```json
{ "location": "강남역", "radius": 500, "categories": ["지하철역", "주차장", "은행"] }
```

#### 11. `estimate_rent` - 임대료 시세 조회
지역별 상가 임대료 시세를 조회합니다. 보증금, 월 임대료, 관리비 추정 및 비용 절감 팁. *(통계 기반 추정)*

```json
{ "location": "강남역", "size": 15, "floor": "1층", "building_type": "상가" }
```

#### 12. `simulate_revenue` - 매출 시뮬레이션
업종별 예상 매출을 시뮬레이션합니다. 일/월/연 매출, 계절별 변동, 예상 순이익 분석. *(통계 기반 추정)*

```json
{ "business_type": "카페", "region": "강남", "size": 15, "staff_count": 1, "operating_hours": 12 }
```
...
| 도구 | 데이터 소스 | 신뢰도 |
|------|-------------|--------|
| 상권 분석 | SEMAS + 카카오 API | 🟢 높음 (실시간) |
| 상권 비교 | 카카오 API | 🟢 높음 (실시간) |
| 경쟁업체 검색 | SEMAS + 카카오 API | 🟢 높음 (실시간) |
| 정책지원금 | 기업마당 API | 🟢 높음 (실시간) |
| 주변 편의시설 | 카카오 API | 🟢 높음 (실시간) |
| 창업 체크리스트 | 카카오 + 정적 | 🟢 높음 |
| 창업 비용 | 통계 기반 추정 | 🟡 중간 |
| 손익분기점 | 통계 + 경쟁 데이터 | 🟡 중간 |
| 임대료 시세 | 통계 기반 추정 | 🟡 중간 |
| 매출 시뮬레이션 | 통계 기반 추정 | 🟡 중간 |

> ⚠️ 🟡 중간 신뢰도 도구는 업종별 평균 통계 기반 추정치입니다. 실제 창업 시 현장 조사를 권장합니다.
...
│   ├── nearby-facilities.ts # 주변 편의시설 분석 (신규)
│   ├── rent-estimate.ts     # 임대료 시세 조회 (신규)
│   └── revenue-simulation.ts # 매출 시뮬레이션 (신규)
├── utils/
...
3. **수익성 분석** - 손익분기점, 투자회수기간, 매출 시뮬레이션
4. **입지 분석** - 임대료 시세, 주변 편의시설, 접근성 평가
5. **맞춤 추천** - 창업자 조건별 지원금/상권 추천
6. **API 캐싱** - 빠른 응답 + 비용 절감
7. **한글 UX** - 모든 메시지 한글화
